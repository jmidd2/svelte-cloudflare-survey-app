            ...
            <div class="p-2 flex flex-col">
                ...
                {#if isHtmlFormField(field.type)}
                    ...
                    {:else if field.type === 'textarea'}
                        <textarea name={field.label.toLowerCase().replaceAll(' ', '-')} id={field.label.toLowerCase().replaceAll(' ', '-')}></textarea>
                    {:else if field.type === 'select'}
                        <select name={field.label.toLowerCase().replaceAll(' ', '-')} id={field.label.toLowerCase().replaceAll(' ', '-')}>
                            {#if field.options}
                    ...
                    {:else}
                        <input name={field.label.toLowerCase().replaceAll(' ', '-')} id={field.label.toLowerCase().replaceAll(' ', '-')} type={field.type}
